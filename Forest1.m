...
sigma = 4*m^2/pi;
A = linspace(min(sqrt(hh(:).*conj(hh(:)))), max(sqrt(hh(:).*conj(hh(:)))), 50);
...
tightfig;
print('-dtiff','-r300',['Results\','Forest1_real'])
%% £¨b£©simulated data
figure; imagesc(20.*log10(A_simu)); axis equal tight off; colorbar; colormap('gray'); caxis([c1,c2]);
tightfig;
print('-dtiff','-r300',['Results\','Forest1_simulated'])
%% £¨c£©Amplitude distribution
figure;
bar(A,P_A_simu,'Linewidth',0.5,'Facecolor','g'); axis square
hold on; bar(A,P_A_real,'Linewidth',0.5,'Facecolor','[0 0.447 0.741]'); 
hold on; plot(A,P_A,'Linewidth',2,'color',[0.85 0.33 0.1]);
legend({'Rayleigh-simulation','Rayleigh-truth','Rayleigh-theory'},'FontSize',15)
title(['Amplitude N=',num2str(N),' \sigma=',num2str(sigma)],'FontSize',15)
ylim([0,max([P_A(:);P_A_simu(:)])]); 
xlim([0,120]);
set(gca,'FontSize',15)
tightfig;
print('-dtiff','-r300',['Results\','Forest1_Amp'])
...
legend({'Rayleigh-simulation','Rayleigh-truth','Rayleigh-theory'},'FontSize',15)
title(['Phase N=',num2str(N),' \sigma=',num2str(sigma)],'FontSize',15)
ylim([0,0.5]); xlim([-pi,pi]);
set(gca,'FontSize',15)
tightfig;
print('-dtiff','-r300',['Results\','Forest1_Phase'])
...
set(gca,'FontSize',8)
print('-dtiff','-r300',['Results\','Forest1_AmpCorr'])
