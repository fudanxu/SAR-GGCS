...
% z = abs(g);
% n_v = 5;
function rho = Cof_estimate2(z,n_v_r,n_v_c)
M = size(z,1);
N = size(z,2);
n_c = fix(N/(2*n_v_c));
n_f = fix(M/(2*n_v_r));
x = zeros(n_v_r,n_v_c,n_f*n_c);
for p = 0:(n_f-1)
    for q = 0:(n_c-1)
        x(:,:,p*n_c+q+1) = z(2*n_v_r*p+1:n_v_r*(2*p+1),2*n_v_c*q+1:n_v_c*(2*q+1));
    end
end
m = mean(x,3);
mm = m(:,:,ones(1,n_f*n_c));
...
xx = x - mm;
cov_z = zeros(n_v_r*n_v_c,n_v_r*n_v_c);
for n = 1:n_f*n_c
    temp = reshape(xx(:,:,n),[],1);
